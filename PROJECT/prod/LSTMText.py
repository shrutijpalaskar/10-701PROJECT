...
class TextLSTM(object):
    ...
    def buildLSTM(self, numEpochs=NUM_EPOCHS, numFeatures=NUM_FEATURES, isBatchNormalized=False,
        ...
        """ Build model """
        inputLayer = lasagne.layers.InputLayer(shape=(None, 35, numFeatures))
        ...
        self.targetValues = T.imatrix('target_output')
    ...
    def calculateAccuracy(self, predictions, truth):
        ...
        :return: accuracy (%)
        """
        numRow, numCol = predictions.shape[0], predictions.shape[1]

        numMatched = 0
        for i in range(numRow):
            # print("Comparing {0} and {1}".format(predictions[i, :], truth[i, :]))
            numMatched += np.array_equal(predictions[i, :], truth[i, :])

        return numMatched / float(numRow)
